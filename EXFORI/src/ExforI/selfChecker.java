...
public class selfChecker<T> {
    ...
    private boolean doCheckReaction(int ii, ObservableList<editableData> myData,
        ...
        for (int i1 = 0; i1 < jCnt; i1++) {
            ...
            String mesg = sIN + "   ----> ";
            boolean atSUMOK = false;
            boolean massSUMOK = false;
            if (rei.atNumReactants == rei.atNumProducts) {
                mesg += " Atomic Number matching....";
                atSUMOK = true;
            } else {
                ReacStatus = "Atomic number sum is not matching..";
            }
            if (rei.massNumReactants == rei.massNumProducts) {
                mesg += " Mass Number matching....";
                massSUMOK = true;
            }else {
                ReacStatus = "Mass number sum is not matching..";
            }
