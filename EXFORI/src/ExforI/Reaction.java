...
public class Reaction {
    ...
    private void getReacParams(libList lList) {
        ...
        if ( sComps.length >= 3 ) {   // for metastable etc
            ...
            System.out.println (
                    sIN + " " + Integer.parseInt (sComps[0]) + "  " + Integer.
                    parseInt (sComps[2]));
        }
        //------------- SF1
        ...
        if ( sComps.length == 1 ) {
            for ( Object myItem : lList.particleAtMassList ) {
                String cmpStr = myItem.toString ().substring (0, myItem.
                        toString ().indexOf (" "));
                if ( cmpStr.contains (sComps[0]) && (cmpStr.length () ==
                        sComps[0].length ()) ) {
                    String tmp = myItem.toString ().substring (myItem.
                            toString ().indexOf (" ")).trim ();
                    sComps = tmp.split ("-");
                    atNumReactants += Integer.parseInt (sComps[0]);
                    massNumReactants += Integer.parseInt (sComps[2]);
                    System.out.println (
                            sIN + " " + Integer.parseInt (sComps[0]) + "  " +
                            Integer.parseInt (sComps[2]));
                }
            }
            ...
            for ( Object myItem : lList.particleAtMassList ) {
                ...
                if ( cmpStr.contains (sComps[0]) && (cmpStr.length () ==
                    ...
                    String tmp = myItem.toString ().substring (myItem.
                            toString ().indexOf (" ")).trim ();
                    sComps = tmp.split ("-");
                    atNumProducts += Integer.parseInt (sComps[0]);
                    massNumProducts += Integer.parseInt (sComps[2]);
                    System.out.println (
                            sIN + " " + Integer.parseInt (sComps[0]) + "  " +
                            Integer.parseInt (sComps[2]));
                    is33 = true;
        ...
        if ( sComps.length == 1 ) {
            ...
            if ( !is236 && !is33 ) {
                if ( exforUtil.isNumeric (sComps[0]) ) {
                    atNumProducts += Integer.parseInt (sComps[0]);  // changed to +=
                    chkReacBalance = true;
                }
                if ( exforUtil.isNumeric (sComps[2]) ) {
                    massNumProducts += Integer.parseInt (sComps[2]);   // changed to +=
                    chkReacBalance = true;
                } else {
                    chkReacBalance = false;
                }
            ...
            System.out.println (
                    ...
                    parseInt (sComps[2]));
            System.out.println (
                    atNumReactants + "   " + massNumReactants + "  " +
                    atNumProducts + "   " + massNumProducts);
        }
        //--------------------- SF4 -----------------------------------
    }
