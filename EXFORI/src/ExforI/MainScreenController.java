...
public class MainScreenController { //implements Initializable {
    ...
    // Variables etc
    private int millisec2Sec = 1000;
    private int sec2Min = 60;
    private int backupFreq = 2; // 2 min
    ...
    private final ObservableList<String> FixedHeadList = FXCollections.
            observableArrayList();
    private final ObservableList<String> TreeHeadList = FXCollections.
            observableArrayList();
...
//FXML stuffs
    ...
    private void startTree() {
        ...
        subEntTree.addEventHandler(MouseEvent.MOUSE_CLICKED, mouseEventHandle);
    ...
    /**
     * This acquires actual context data from myData using key word sent from
     * clicking the tree head
     *
     * @param sIN
     * @return
     */
    private String sendTextForPopup(String sIN) {
        String forReturn = "";
        for (int ii = 0; ii < myDataN - 1; ii++) {
            if (myData.get(ii).getBibItemName().contains(sIN)) {
                forReturn = myData.get(ii).getContentTxt();
            }
        }
        return forReturn;
    }
    ...
    /**
     *
     * @param sIN1 input keyHeader
     * @param sIN2 input data for key
     */
    private void fixTableWithTree(String sIN1, String sIN2) {
    ...
    private void getbackUPFreq() {
        backupDelay = backupFreq * sec2Min * millisec2Sec;
    }
    ...
    private void CreateHeads() {
        ...
        TreeHeadList.addAll("TITLE", "AUTHOR", "INSTITUTE",
                "REFERENCE", "FACILITY");
        TreeHeadList.addAll("SAMPLE", "INC-SOURCE", "DETECTOR", "MONITOR",
                "MONIT-REF", "DECAY-MON");
        TreeHeadList.addAll("REL-REF", "CORRECTION", "ADD-RES", "STATUS",
                "HISTORY", "COMMON", "NOCOMMON");
...
//                "chicken and vegetable. We like Chicken from KFC";
    ...
    private void doEditFacility(int ri, String Head, String act, String SE) {
        ...
        if (!act.contains("Delete")) {
            ...
            selectIt.setOnAction((ActionEvent event1) -> {
                ...
                String tempB = "";
                String tempC = "";
                tempA = (!facilCB.getValue().isEmpty()) ? facilCB.getValue() : "";
                tempA = (!tempA.isEmpty()) ? tempA.substring(0, tempA.indexOf(" ")) : "";
                tempB = (!institute.getValue().isEmpty()) ? institute.getValue() : "";
                tempB = (!tempB.isEmpty()) ? tempB.substring(0, tempB.indexOf(" ")) : "";
            ...
            acceptEdit.setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event1) {
                    ...
                    if (!isSelect) {
                        ...
                        System.out.println("0 " + parseStr + "lCnt1 " + lCnt1 +" multiLine "
                                + multiLine
                        );
                        //parseStr = arrTitle (parseStr);
    ...
    /**
     * To fix head style. now defunct as creating problem. Care is given using
     * CSS
     */
    ...
    // This is code for an attempt to make tree in action
    /**
     *
     * @param event
     */
    private void handleMouseClicked(MouseEvent event) {
        /*
         * Node node = event.getPickResult().getIntersectedNode(); // Accept
         * clicks only on node cells, and not on empty spaces of the TreeView if
         * (node instanceof Text || (node instanceof TreeCell && ((TreeCell)
         * node).getText() != null)) { String name = (String) ((TreeItem)
         * subEntTree.getSelectionModel().getSelectedItem()).getValue();
         * System.out.println("Node click: " + name);
         * System.out.println(bibList.size()); for (int ii = 0; ii <
         * bibList.size(); ii++) { String tmp1 = bibList.get(ii).getStrItem();
         * if (tmp1.matches(name)) { String toSend = sendTextForPopup(tmp1);
         * String toReturn = popupMsg.showAndEditMsg(toSend);
         * System.out.println(toSend + " is sent to popup");
         * System.out.println("return from popup-->" + toReturn);
         *
         * }
         * }
         * }
...
//@SuppressWarnings ("unchecked")
    ...
    public void setDefaultDirExt(String fPath) {
