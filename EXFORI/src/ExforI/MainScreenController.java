...
//                "chicken and vegetable. We like Chicken from KFC";
    ...
    private void handleMouseClicked(MouseEvent event) {
        ...
        if (node instanceof Text || (node instanceof TreeCell && ((TreeCell) node).getText() != null)) {
            String name = (String) ((TreeItem) subEntTree.getSelectionModel().getSelectedItem()).getValue();
            System.out.println("Node click: " + name);
            System.out.println(bibList.size());
            for (int ii = 0; ii < bibList.size(); ii++) {
                String tmp1 = bibList.get(ii).getStrItem();
                if (tmp1.matches(name)) {
                    String toSend = sendTextForPopup(tmp1);
